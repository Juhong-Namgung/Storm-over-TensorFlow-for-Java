...
public class MyKafkaBolt<K, V> extends BaseTickTupleAwareRichBolt {
    ...
    protected void process(final Tuple input) {
        ...
        try {
        	...
        	//LOG.info("Current Time: " +  System.currentTimeMillis()+ "  ////// Start-time:  " + input.getLongByField("start-time"));
        	lead_time += System.currentTimeMillis() - input.getLongByField("start-time");
        	rcv_number_of_tuples++;
        	tuple_size += input.size();
        	total_tuple_size += input.getLongByField("size");
    ...
    @Override
    public void cleanup() {
    	LOG.info("\n$$$$ Bolt cleanup,total_rcv_number_of_tuples: " + rcv_number_of_tuples+" /// avg_time,"+((double)lead_time/(double)rcv_number_of_tuples) + " /// Kafkabolt TUPLE SIZE: " +  tuple_size + " ///Total Tuple Size: " + total_tuple_size);
        producer.close();
    }
