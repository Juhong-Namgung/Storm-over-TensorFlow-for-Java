...
public class PreProcessingBolt extends BaseRichBolt {
	...
	public void execute(Tuple input) {
		...
		try (Tensor<Float> image = constructAndExecuteGraphToNormalizeImage(imageBytes)) {					
			image.copyTo(imageTensor);
			
			
			
			/*Kryo kryo = new Kryo();
			com.esotericsoftware.kryo.io.Output output = new com.esotericsoftware.kryo.io.Output(image.bytesValue());
			kryo.writeClassAndObject(output, image.bytesValue());
			*/
			this.collector.emit(input, new Values(imageBytes, imageTensor, input.getLongByField("start-time")));
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		declarer.declare(new Fields("img", "tensor", "start-time"));
	}
