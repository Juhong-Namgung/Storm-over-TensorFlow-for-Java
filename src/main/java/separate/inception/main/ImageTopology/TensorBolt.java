...
public class TensorBolt extends BaseRichBolt {
	...
	private byte[] graphDefs;
	private String modelDir = "/home/team1/juhong/kepco/tensorflowforjava/resultmodel/inception5h";
	private String modelName = "tensorflow_inception_graph.pb";
	
	private List<String> labels;
	private float[][][][] imageTensor;
	
	private long latency;
	
	static long count;
	private Tensor<Float> image;
	...
	public void prepare(Map stormConf, TopologyContext context, OutputCollector collector) {
		...
		this.count = 0L;
		this.image = null;
		this.imageTensor = new float[1][224][224][3];
	...
	public void execute(Tuple input) {
		...
		try {
			...
			imageTensor = (float[][][][]) input.getValueByField("tensor");
			//image = (Tensor<Float>)input.getValueByField("tensor"); 
			image = (Tensor<Float>) Tensor.create(imageTensor);
...
//			System.out.println("Count: " + count + "// Latency: " + latency);
			//count ++;
			
			this.collector.emit(input, new Values(input.getBinaryByField("img"), labels.get(bestLabelIdx), labelProbabilities[bestLabelIdx] * 100f, input.getLongByField("start-time")));
