...
public class SetGroupingPerfTopo {

    private static final String TOPOLOGY_NAME = "PerfImageTopo";
    ...
    private static StormTopology getTopology(Map conf) {
        ...
        // 1 -  Setup Spout   --------
        FirstSpout spout = new FirstSpout("/home/team1/juhong/kepco/tensorflowforjava/sampledata/",  Helper.getInt(conf, "message.interval", 50));
    ...
    private static void setGrouping(BoltDeclarer bolt, Map conf, String prevTaskID) {
		...
		case "local":
			...
			break;
		/*case "locality":
			LocalityAwareGrouping localityAwareGrouping = new grouping.locality.improved.LocalityAwareGrouping(
					zkConnection, Helper.getInt(conf, "zookeeper.session.timeout", 20000));
			localityAwareGrouping.setWeight(Helper.getStr(conf, "weight.function"),
					Helper.getStr(conf, "weight.param"));
			localityAwareGrouping.setRttInterval(Helper.getInt(conf, "rtt.interval", 60));
			bolt.customGrouping(prevTaskID, localityAwareGrouping);
			break;*/
		}
	}
